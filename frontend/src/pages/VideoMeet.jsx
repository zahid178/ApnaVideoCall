...
export default function VideoMeetComponent() {
    ...
    let [videoAvailable, setVideoAvaliable] = useState(true);
    let [audioAvailable, setAudioAvaliable] = useState(true);
    let [video, setVideo] = useState([]);
    let [audio, setAudio] = useState();
    let [screen, setScreen] = useState();
    // let [showModal, setModal] = useState(true);
    let [showModal, setModal] = useState(false);
    let [screenAvalaible, setScreenAvalaible] = useState();
    ...
    const getPermissions = async () => {
     try{
        ...
        if(navigator.mediaDevices.getDisplayMedia) {
          setScreenAvalaible(true);
        } else {
          setScreenAvalaible(false);
        }
...
// todo
  ...
  // Temporary ICE candidate storage
...
let gotMessageFromServer = (fromId, message) => {
    ...
    connection.setRemoteDescription(new RTCSessionDescription(signal.sdp)).then(() => {
      ...
      if (pendingCandidates[fromId]) {
        pendingCandidates[fromId].forEach(candidate => {
          connection.addIceCandidate(candidate).catch(e => console.log("Pending ICE error:", e));
        });
  ...
  if (signal.ice) {
    ...
    if (connection.remoteDescription && connection.remoteDescription.type) {
      connection.addIceCandidate(candidate).catch(e => console.log("ICE add error:", e));
    } else {
      // Queue the candidate until remoteDescription is ready
